...
async function callGemini(parts, retryCount = 0) {
  const model = $("modelName").value.trim() || "gemini-1.5-flash-latest";
  const key = nextKey();
...
function normalizeItem(raw = {}) {
  return {
    ...
    category: raw.category || "確認",
    status: raw.status || "要確認",
    confidence: raw.confidence || "低",
...
async function openDb() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open("gijiroku_db_v02", 1);
...
async function resizeDataUrl(dataUrl, mimeType, maxSize) {
  ...
  return canvas.toDataURL(outType, 0.88);
}
...
async function handleGeminiRead() {
  if (!apiKeys.length) {
    alert("先に gemini_api_key.txt を読み込んでください。");
    return;
  }
  if (!images.length) {
    alert("先に図面・手書き画像を追加してください。");
    return;
  }
  ...
  try {
    ...
    log("Gemini応答を受信しました。JSONを解析します。");
    const data = parseGeminiJson(text);
    items = (data.items || []).map(normalizeItem);
    if (!items.length && data.summary) {
      items = [normalizeItem({room:"全体", target:"要約", content:data.summary, category:"確認", status:"要確認", confidence:"中", reason:"Gemini応答にitemsがなかったため要約を仮登録"})];
    }
    renderItems();
    await saveProject();
...
<thead>
<tr>
<th>No.</th><th>部屋</th><th>対象</th><th>内容</th><th>区分</th><th>状態</th><th>信頼度</th><th>備考</th>
</tr>
</thead>
<tbody>
${rows || `<tr><td colspan="8">項目なし</td></tr>`}
</tbody>
</table>
</body>
</html>`;
...
function initEvents() {
  ...
  $("testGeminiBtn").addEventListener("click", async () => {
    ...
    try {
      log("接続テスト開始。");
      const text = await callGemini([{text:'{"test":"接続テストです。JSONで {\\"ok\\":true} の形式だけ返してください。"}'}]);
...
async function init() {
  ...
  renderImages();
  renderItems();
  log("起動しました。まず gemini_api_key.txt を読み込んでください。");
}
